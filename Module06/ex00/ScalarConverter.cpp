...
static bool canConvertToChar(double value) {
    return !isSpecial(value) && value > -1.0 && value < 128.0;
}

static bool canConvertToInt(double value) {
    const double lowerLimit = static_cast<double>(INT_MIN) - 1.0;
    const double upperLimit = static_cast<double>(INT_MAX) + 1.0;
